...
function analyzeComplexity(nodes, edges, boundaries) {
  ...
  const avgPrereqs = nodesWithPrereqs.length > 0 
    ? nodesWithPrereqs.reduce((sum, n) => sum + n.prerequisites.length, 0) / nodesWithPrereqs.length 
    : 0;
...
function calculatePerformanceIndicators(nodes, edges, boundaries) {
  ...
  if (boundaries.area > 1000000) { // 1M square units
