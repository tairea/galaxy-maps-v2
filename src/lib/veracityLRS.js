import { fetchCourseById, fetchStudentByEmail } from "@/lib/ff";
...
async function sanitiseCohortsCourseDataFromLRS(res) {
  ...
  for (const group of res) {
    ...
    const people = await Promise.all(
      group.actors.map((x) => fetchStudentByEmail(x.actor.split(":")[1])),
    );
...
async function sanitiseCohortsActivityDataFromLRS(res) {
  ...
  // Get all the people first
  const people = await Promise.all(res.map((x) => fetchStudentByEmail(x._id.actor.split(":")[1])));
...
async function courseIRIToCourseId(course) {
  ...
  // get course name
  const courseContext = await fetchCourseById(courseId);
