...
const { getAuth: getAdminAuth, connectAuthEmulator: connectAdminAuthEmulator } = require('firebase-admin/auth');
...
async function importTestData() {
  ...
  try {
    ...
    for (const [docId, docData] of Object.entries(testData.people)) {
      if (docId !== 'admin1') { // Skip admin1 as it was already created
    ...
    console.log(`  ✅ Help Requests: ${Object.keys(testData.requestsForHelp).length} courses with requests`);
    ...
    for (const [courseId, submissions] of Object.entries(testData.submissionsForReview)) {
      for (const [submissionId, submissionData] of Object.entries(submissions)) {
        await setDoc(doc(db, 'courses', courseId, 'submissionsForReview', submissionId), submissionData);
      }
    }
    console.log(`  ✅ Submissions: ${Object.keys(testData.submissionsForReview).length} courses with submissions`);
...
async function testStudentAccess() {
  ...
  try {
    ...
    const studentCredential = await signInWithEmailAndPassword(auth, 'student1@example.com', 'student123');
    ...
    try {
      ...
      if (otherUserDoc.exists()) {
        ...
        if (data.firstName && data.email && data.lastName) {
          logTest('Student read profile info of others', true, 'Can access firstName, email and lastName');
        } else {
...
async function testTeacherAccess() {
  ...
  try {
    ...
    const teacherCredential = await signInWithEmailAndPassword(auth, 'teacher1@example.com', 'teacher123');
    ...
    try {
      ...
      if (error.code === 'permission-denied') {
        ...
        logTest('Teacher update student profile they teach', false, `Unexpected error: ${error.message}`);
      }
    }
    ...
    try {
      ...
      if (error.code === 'permission-denied') {
        ...
        logTest('Teacher1 read other teacher private course', false, `Unexpected error: ${error.message}`);
      }
    }
    ...
    try {
      ...
      logTest('Teacher1 update other teacher public course', false, 'Should be denied - not owner/contentBy');
    } catch (error) {
      if (error.code === 'permission-denied') {
        logTest('Teacher1 update other teacher public course', true, 'Correctly denied');
      } else {
        logTest('Teacher1 update other teacher public course', false, `Unexpected error: ${error.message}`);
      }
    }
    ...
    try {
      ...
      logTest('Teacher1 delete other teacher course', false, 'Should be denied - not owner/contentBy');
    } catch (error) {
      if (error.code === 'permission-denied') {
